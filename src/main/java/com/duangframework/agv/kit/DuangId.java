 ...
 * @date  2017/11/21.
 */
 public final class DuangId implements Comparable<DuangId>, Serializable {


    private static final long serialVersionUID = 3670079982654483072L;

    static final Logger LOGGER = LoggerFactory.getLogger(DuangId.class);
     ...
     * @return the new id
     */
    public static DuangId get() {
        return new DuangId();
    }

    /**
     * Checks if a string could be an {@code DuangId}.
     *
     ...
     * @since 2.12.0
     */
    public static DuangId createFromLegacyFormat(final int time, final int machine, final int inc) {
        return new DuangId(time, machine, inc);
    }
     ...
     * @param date the date
     */
    public DuangId(final Date date) {
     ...
     */
    public DuangId(final Date date, final int counter) {
     ...
     */
    public DuangId(final Date date, final int machineIdentifier, final short processIdentifier, final int counter) {
     ...
     * @throws IllegalArgumentException if the high order byte of machineIdentifier or counter is not zero
     */
    public DuangId(final int timestamp, final int machineIdentifier, final short processIdentifier, final int counter) {
        this(timestamp, machineIdentifier, processIdentifier, counter, true);
    }

    private DuangId(final int timestamp, final int machineIdentifier, final short processIdentifier, final int counter,
     ...
     */
    public DuangId(final String hexString) {
     ...
     */
    public DuangId(final byte[] bytes) {
        if (bytes == null) {
            throw new IllegalArgumentException();
        }
        if (bytes.length != 12) {
     ...
     */
    DuangId(final int timestamp, final int machineAndProcessIdentifier, final int counter) {
...
//        StringBuilder buf = new StringBuilder(24);
        StringBuilder buf = createIdStringBuilder();
    ...
    private StringBuilder createIdStringBuilder() {
    ...
    @Override
    public int compareTo(final DuangId other) {
        if (other == null) {
